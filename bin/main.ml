...
let run input output follow_remote force_inline force_skip score_threshold min_links =
  ...
  let read_input path =
    try let ic = open_in path in let s = really_input_string ic (in_channel_length ic) in close_in ic; Ok s with e -> Error (Printexc.to_string e)
  in
...
let input_t = Arg.(required & pos 0 (some Cmdliner.Arg.filepath) None & info [] ~docv:"INPUT" ~doc:"Input markdown path")
